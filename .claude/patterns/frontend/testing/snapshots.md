...
**Path**: `src/libs/infrastructure/repositories/business/enrollment-management/enrollment.repository.ts`
**Lines**: ~280 | **Read Priority**: Medium

**Contains**:
- `create`: Insert new enrollment
- `findById`: Get single enrollment
- `findByUserId`: Get user's enrollments
- `findByUserIdWithEvents`: Get enrollments with event data (JOIN)
- `update`: Update enrollment fields
- Prisma ↔ Entity transformers

**Read this file when**:
- Adding new query method
- Changing database schema interaction
- Understanding what data is fetched

**Skip this file when**:
- Only need to call existing methods (see Use Case)
- Only need API response format (see API route)
```
...
- `.claude/patterns/context-first-patterns.md` - When/how to use snapshots

**Other Patterns**:
- Snapshots REFERENCE patterns, they are NOT patterns
- Patterns have code examples, snapshots don't

**Standards**:
- Snapshots REFERENCE standards
- Standards have theory, snapshots have state

---

**Lines**: ~350
**Type**: Meta-documentation pattern
**Version**: 2.0 | **Updated**: 2026-01-27
