...
/**
 * English mock data patterns — Spanish locale is mandatory for all test data.
 * Detects common English names, emails, and placeholder data.
 */
const ENGLISH_MOCK_DATA_PATTERNS = [
  /\bJohn\b/,
  /\bJane\b/,
  /\bDoe\b/,
  /\bSmith\b/,
...
export const essentialTestingRule = {
  meta: {
    ...
    messages: {
      ...
      englishMockData:
        'English mock data detected: "{{value}}". Use Spanish locale data (María García, José López, etc.).',
    },
    schema: [
      {
        type: 'object',
  ...
  create(context) {
    ...
    function checkEnglishMockData(node) {
      if (node.type === 'Literal' && typeof node.value === 'string') {
        for (const pattern of ENGLISH_MOCK_DATA_PATTERNS) {
          if (pattern.test(node.value)) {
            context.report({
              node,
              messageId: 'englishMockData',
