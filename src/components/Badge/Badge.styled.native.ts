/**
 * Badge Styled Component — React Native resolution
 *
 * Span primitive (Text): text children render directly; padding, background
 * and border-radius are all Text-safe on native.
 */

import styled, { css } from 'styled-components/native';

import { c, s, sh, tf, ts, tw } from '../../tokens/css-variables';
import { Span } from '../../primitives';
import type { StyledBadgeProps } from './Badge.interfaces';

const getVariantStyles = ($variant: StyledBadgeProps['$variant']) => {
  switch ($variant) {
    case 'primary':
      return css`
        background-color: ${c('primary100')};
        color: ${c('primary700')};
      `;
    case 'success':
      return css`
        background-color: ${c('successBackground')};
        color: ${c('successDark')};
      `;
    case 'warning':
      return css`
        background-color: ${c('warningBackground')};
        color: ${c('warningDark')};
      `;
    case 'danger':
      return css`
        background-color: ${c('errorBackground')};
        color: ${c('errorDark')};
      `;
    case 'info':
      return css`
        background-color: ${c('secondary100')};
        color: ${c('secondary700')};
      `;
    case 'secondary':
      return css`
        background-color: ${c('secondary50')};
        color: ${c('secondary600')};
      `;
    case 'default':
    default:
      return css`
        background-color: ${c('neutral100')};
        color: ${c('neutral700')};
      `;
  }
};

const getSizeStyles = ($size: StyledBadgeProps['$size']) => {
  switch ($size) {
    case 'sm':
      return css`
        font-size: ${ts('xs')};
        padding: 2px ${s('xs')};
      `;
    case 'lg':
      return css`
        font-size: ${ts('sm')};
        padding: ${s('xs')} ${s('md')};
      `;
    case 'md':
    case undefined:
    default:
      return css`
        font-size: ${ts('xs')};
        padding: ${s('micro')} ${s('sm')};
      `;
  }
};

export const StyledBadge = styled(Span)<StyledBadgeProps>`
  align-self: flex-start;
  border-radius: ${sh('full')};
  font-family: ${tf('body')};
  font-weight: ${tw('medium')};
  overflow: hidden;
  ${({ $size }) => getSizeStyles($size)}
  ${({ $variant }) => getVariantStyles($variant)}
`;
