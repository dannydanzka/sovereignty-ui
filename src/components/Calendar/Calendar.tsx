/**
 * Calendar Component — web resolution.
 *
 * A month-grid date picker for a single date or a range. Days can be blocked via
 * `isDateDisabled` (e.g. no inventory), so it doubles as an availability picker.
 * Pure date math lives in Calendar.helpers.ts (shared with the native resolution).
 */
...
export const Calendar = ({
  ...
  return (
    <CalendarWrapper className={className}>
      <CalendarHeader>
        <NavButton aria-label={prevMonthLabel} type='button' onClick={goPrev}>
          <ChevronLeft size={18} />
        </NavButton>
        <CalendarTitle>{monthLabel(visibleMonth, locale)}</CalendarTitle>
        <NavButton aria-label={nextMonthLabel} type='button' onClick={goNext}>
