...
- 54f74b5: Card: the polymorphic prop is now `element`, not `as` — because `as` never worked through a wrapper.
...
- 2d0b1d9: `Card` gains a polymorphic `as`, and its radius comes from the shape token instead of a hardcoded `12px`.

  **`as`** — a card is a _surface_, not a _meaning_, and the meaning belongs to the caller: a card in a
...
li | section` and keeps every surface style.
  ...
  **Radius** — same rendered value (`shape.lg` _is_ `0.75rem`), but a literal pixel inside the design
...
- 76ea0ec: New `Chip`, and `Tabs` completes the ARIA tabs contract.
...
- 956ec9f: Record-data primitives: new `TotalsList`, plus a colour seam on `ProgressBar` and `Avatar`.
...
- 8ac2d4c: `Textarea` and `Select` now render the `*` required marker that `Input` has always rendered.
